...
const DOC_TYPES = [
  { value: 'land_deed', label: 'Land Deed', color: 'bg-amber-100 text-amber-700' },
  { value: 'soil_card', label: 'Soil Health Card', color: 'bg-green-100 text-green-700' },
  { value: 'identity', label: 'Identity (Aadhaar/PAN)', color: 'bg-blue-100 text-blue-700' },
          ...
          return (
            <Card key={d.id}><CardContent className="pt-3">
              <div className="flex items-start gap-3">
                {d.file_url ? <Image src={d.file_url} className="h-16 w-16 rounded-lg shrink-0" fittingType="fill" /> : <div className="h-16 w-16 rounded-lg bg-gray-100 flex items-center justify-center shrink-0"><FileText className="h-6 w-6 text-gray-400" /></div>}
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">{d.issued_date ? `Issued: ${d.issued_date}` : ''}{d.expiry_date ? ` · Expires: ${d.expiry_date}` : ''}</p>
      ...
      {showAdd ? (
        <Card className="border-green-200"><CardContent className="pt-4 space-y-3">
          ...
          <label className="cursor-pointer">
            <span className="inline-flex items-center gap-1 text-xs bg-blue-50 text-blue-700 px-2.5 py-1.5 rounded-lg">{uploading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />}{t('uploadFile')}</span>
        ...
        <Button onClick={() => setShowAdd(true)} variant="outline" className="w-full border-green-300 text-green-700"><Plus className="h-4 w-4 mr-1" />{t('addDocument')}</Button>
      )}
    </div>
  );
}
