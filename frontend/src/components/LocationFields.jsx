    ...
    <div className={compact ? 'grid grid-cols-2 gap-2' : 'space-y-2'}>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <Input value={value.mandal || ''} onChange={(e) => setMandal(e.target.value)} placeholder="Mandal" />
        </div>
        <div>
          <Label className="text-sm">Village</Label>
          <Input value={value.village || ''} onChange={(e) => setVillage(e.target.value)} placeholder="Village" />
        </div>
      </div>
    </div>
  );
